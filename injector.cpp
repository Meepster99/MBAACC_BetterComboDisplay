...
int main() {
	...
	*(std::strrchr(buffer, '\\') + 1) = '\0';
	...
	std::string source = std::string(buffer) + "BetterComboDisplay.dll";
	std::string dest = path + "BetterComboDisplay.dll";

	if(!fileExists(dest)) {
		exitWithCode(1, "unable to find BetterComboDisplay.dll in melty path. copy it in");
	}
